...
const ChatPage: React.FC = () => {
  ...
  const handleToggleWatchlist = useCallback(
    async (stockCode: string) => {
      ...
      try {
        ...
        if (isMountedRef.current) {
          setWatchlistMessage(t('chat.watchlistError'));
        }
      } finally {
        if (isMountedRef.current) {
  ...
  useEffect(() => {
    ...
    void systemConfigApi.getConfig(false)
      ...
      .catch((error) => {
        ...
        setContextCompressionError(parsed.message || t('chat.contextCompressionReadError'));
  ...
  const updateContextCompressionEnabled = useCallback(
    async (nextEnabled: boolean) => {
      ...
      try {
        ...
        setContextCompressionError(parsed.message || t('chat.contextCompressionSaveError'));
      } finally {
  ...
  const handleSend = useCallback(
    async (overrideMessage?: string, overrideSkillIds?: string[]) => {
      ...
      const usedSkillNames = usedSkillIds.length > 0 ? getSkillNames(usedSkillIds) : [t('chat.generalAnalysis')];
  ...
  const downloadMessageAsMarkdown = useCallback((msg: Message) => {
    ...
    const heading = msg.role === 'user' ? `# ${t('chat.userMessage')}` : `# ${t('chat.aiReply')}${skillLabel ? ` · ${skillLabel}` : ''}`;
      ...
      <ScrollArea testId="chat-session-list-scroll" viewportClassName="p-3">
        {sessionsLoading ? (
          ...
          <DashboardStateBlock
            ...
            description={t('chat.noSessionsDesc')}
            className="rounded-2xl border border-dashed border-border/50 bg-surface/30"
          />
        ) : (
          ...
          <div className="border-t border-white/6 bg-card/88 p-4 md:p-6 relative z-20">
            <div className="space-y-3">
              ...
              {isFollowUpContextLoading ? (
                <InlineAlert
                  variant="info"
                  title={t('chat.followUpLoading')}
                  message={t('chat.followUpLoadingDesc')}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/6 bg-surface/25 px-3 py-2">
                ...
                >
                  ...
                  <span className="text-xs text-muted-text">{t('chat.contextCompressionDesc')}</span>
                </label>
              ...
              {contextCompressionError ? (
                <InlineAlert
                  variant="danger"
                  title={t('chat.contextCompressionError')}
            ...
            {activeStockCode && (
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  className="text-[11px]"
                >
                  {stockInWatchlist(activeStockCode) ? t('chat.removeFromWatchlist') : t('chat.addToWatchlist')}
                </Button>
                {watchlistMessage && (
