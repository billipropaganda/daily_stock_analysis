...
function renderTestResultMessage(result: AlertRuleTestResponse, t: (key: string, params?: Record<string, string | number>) => string): React.ReactNode {
  ...
  return (
    <div className="space-y-2">
      <div>
        ...
        {' · '}{t('alerts.testObservedValue')}{'：'}
        {result.observedValue == null ? '--' : String(result.observedValue)}
      </div>
      {result.evaluatedCount != null && result.evaluatedCount > 1 ? (
        <div className="text-xs">
          {t('alerts.testSummary', { evaluated: result.evaluatedCount, triggered: result.triggeredCount ?? 0, degraded: result.degradedCount ?? 0, skipped: result.skippedCount ?? 0 })}
        </div>
      ) : null}
...
const AlertsPage: React.FC = () => {
  ...
  return (
    <AppPage className="space-y-5">
      ...
      <Card title={t("alerts.notificationRecords")} subtitle={t("alerts.notificationSubtitle")} variant="bordered" padding="md">
        {notificationsLoading ? <Loading label={t("alerts.notificationLoading")} /> : null}
        {!notificationsLoading && notifications.length === 0 ? (
          <EmptyState
            icon={<BellRing className="h-6 w-6" />}
            title={t("alerts.notificationEmpty")}
            description={t("alerts.notificationEmptyDesc")}
          />
        ) : null}
        {!notificationsLoading && notifications.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[680px] text-left text-sm">
              <thead className="border-b border-border/60 text-xs uppercase text-muted-text">
                <tr>
                  <th className="px-3 py-2 font-medium">{t("alerts.tableChannel")}</th>
                  <th className="px-3 py-2 font-medium">{t("alerts.tableStatus")}</th>
                  <th className="px-3 py-2 font-medium">{t("alerts.tableErrorCode")}</th>
                  <th className="px-3 py-2 font-medium">{t("alerts.tableDuration")}</th>
                  <th className="px-3 py-2 font-medium">{t("alerts.tableTime")}</th>
                  <th className="px-3 py-2 font-medium">{t("alerts.tableDiagnostics")}</th>
                </tr>
              </thead>
